            ...
            string status)
        {
                ...
                StatusOptions.Contains(status))
            {
                ...
                if (newlyAvailable.Count > 0)
                {
                    ...
                    {
                        _logger.LogError(exception, "Availability saved but notifications failed for farm {FarmId}.", FarmId);
                        TempData["AvailabilityEmailMessage"] = "Availability saved, but email notification failed. Check email settings and the application log.";
                    }
                }
            }
